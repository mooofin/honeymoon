...
private:
  ...
  const std::vector<std::string> home_menu = {"File Searcher", "Recent Files",
                                              "Settings",      "Help",
                                              "About",         "Quit"},
                                 settings_menu = {"Line Numbers",
                                                  "Syntax Highlighting",
                                                  "Tab Width", "Back"};
  ...
  void draw_state_ui(HomeState &s, int y, int) {
  ...
  void draw_state_ui(RecentFilesState &s, int y, int) {
  ...
  void draw_state_ui(FileSearchState &s, int y, int) {
    draw_centered_text(y, "Search File: " + s.query);
  }
  template <typename T> void draw_state_ui(T &, int, int) {}
  void draw_centered_text(int y, const std::string &text) {
    if (y < window_rows) {
      int w = get_display_width(text), p = std::max(0, (window_cols - w) / 2);
      output_buffer.append(std::format("\x1b[{};{}H", y + 1, p + 1))
          .append(text);
    }
  }
  struct EditorCursor {
  ...
  void handle_input(SettingsState &, Key k) {
    ...
    // ... Settings interaction logic could go here, currently minimal
  }
