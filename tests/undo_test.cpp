...
TEST(UndoTest, RespectsMaxUndoLevels) {
    TestUndo undo;
    for (int i = 0; i < 150; ++i) {
        undo.close_typing_group();   // close group to allow next snapshot
        undo.snapshot_for_undo(std::to_string(i), i);
    }

    // Only the last 100 should remain (max_undo_levels = 100)
    // So we can undo ~100 times plus the current state
    int undo_count = 0;
    while (undo.apply_undo("current", 0).has_value())
        ++undo_count;

    EXPECT_LE(undo_count, 100);
    EXPECT_GE(undo_count, 90);  // may be slightly less due to edge effects
}
